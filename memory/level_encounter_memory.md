...
- **2026-03-15 (Session sign-off):** Attack patterns by CEO name (e.g. Zig-Zag Pressure, Scatter & Converge) drive formation + movement. Level 1: waves 1–3 Zig-Zag Pressure (line formation, zig_zag 135px amplitude, 4s period); waves 4–5 Scatter & Converge (staggered_wedge, scatter_converge 130px spread). +2 scouts per wave (5,6,5,6,7). New formation `line` (horizontal, 9 slots, 60px spacing). Elite is one random slot in the formation and uses same movement behavior as the wave (elite zig-zags with scouts). Reference: docs/design_system/attack_pattern_reference.md.
