...
- **2026-03-04 (CEO):** Core C# First approach approved. Build game logic in pure C# (src/Vimanas.Core) outside Unity; port to Unity later as thin presentation layer. Rationale: Unity verification hard (SpriteRenderer macOS, CI license); agents need hands-on with code and results. Plan: phases A–D; Canvas/UI only for Unity gameplay rendering. Record in specialist memories.
...
- **2026-03-04 (CEO):** GitHub build pipeline disabled. `.github/workflows/build.yml` now triggers only on `workflow_dispatch` (manual). Push/PR no longer run CI. Re-enable by restoring `push:` and `pull_request:` under `on:`.
