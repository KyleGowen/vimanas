...
| **Pilots**   | 7, 9    | Portraits, modifiers, pairing; XP/level design; per [pilot_mocks](docs/concepts/p0_mocks/p0_2_pilots/pilot_mocks_deliverable.md)                                      |
...
| **UI**       | 5, 6, 7, 8 | HUD, results, ship select, hangar; ornate inventor-fair                                                                                                               |
...
| 5     | 4.1   | Tech   | **Vertical scroll** — Camera or world scrolls upward; player ship stays in frame (e.g. bottom third); smooth scroll rate. LevelScrollController or equivalent. Entities use world coordinates.                                                                                                                                 | 4.A.1      | Done   | Smooth scroll; player in frame       | level scrolls; player in frame      |
...
| 8     | 4.4   | Tech   | **Boss placeholder** — Boss prefab in scene; HP bar UI; takes damage; defeat triggers level complete. Visual: block or simple sprite per boss placeholder design.                                                                                                                                                    | 4.A.4, 4.3 | Done ✓  | HP bar; defeat → level complete      | boss appears; defeat → level complete |
...
| Order | ID    | Type   | Deliverable                                                                                                                                                                                         | Depends on | Status  | Gate                              | How to verify                                      |
| ----- | ----- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------- | --------------------------------- | -------------------------------------------------- |
| 1     | 5.A.1 | Design | **HUD design** — Layout (HP bar, mana bar, score, lives); aether accents, filigree framing per [art_style_guide](docs/art_style_guide.md); legible at 1080p. Output: `docs/concepts/hud_design.md`. | —          | Done ✓  | Visual Design spec; design_system | Read hud_design.md; spec approved                  |
| 2     | 5.A.3 | Design | **Results screen design** — Layout; Retry/Continue flow; victory/defeat states. Output: `docs/concepts/results_screen_design.md`.                                                                   | —          | Done ✓  | Design system compliance          | Read results_screen_design.md; spec approved       |
...
| 2     | 5.1.B   | Asset | **Parallax Option B — Procedural** — Generate tileable layers via Canvas/Node script; palette #2d6a2d, #6b8e23, #8fbc8f, earth brown, gold. Use if Option A seams unacceptable.                             | 5.1.A      | Cancelled | CEO preferred 5.1.A              | —                                                  |
...
**Delivery order:** One ship at a time — design lock → primary/secondary weapon + shield design → sprite sheet → ship entity + weapons + shield → integrate; then ship selection (ships only). **No pilots in this phase.** Iterate on each ship before moving to the next.


| Order | ID    | Type   | Deliverable                                                                                                                                                                                                                                             | Depends on   | Status  | Gate                                     | How to verify                                      |
| ----- | ----- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ------- | ---------------------------------------- | -------------------------------------------------- |
| *Turtle* | | | | | | |
| 1     | 6.T.1 | Design | **Turtle design lock** — Stats (high HP/Defense, low Speed); tank silhouette; amber/gold propulsion (#FFBF00). Output: `docs/concepts/p0_mocks/p0_1_ships/turtle/turtle_design_lock.md`. P0 Mocks Considered.                                           | —            | Pending | Per p0_1_ships; design_system            | Read turtle_design_lock.md; CEO approves            |
| 2     | 6.T.2 | Design | **Turtle primary weapon design** — Main gun type, fire pattern, damage, mana cost. Per ship_weapons_design_lock. Tank-appropriate feel.                                                                                                                  | 6.T.1        | Pending | Combat Systems; tank feel                 | Turtle primary weapon spec approved                 |
| 3     | 6.T.3 | Design | **Turtle secondary weapon design** — Alt fire; cooldown, mana cost, effect. Distinct from primary.                                                                                        | 6.T.1        | Pending | Combat Systems; tank feel                 | Turtle secondary weapon spec approved               |
| 4     | 6.T.4 | Design | **Turtle shield design** — Shield strength, recharge rate, visual language; aether/filigree per art_style_guide.                                                                             | 6.T.1        | Pending | Combat Systems + Visual Design spec      | Turtle shield spec approved                         |
| 5     | 6.T.5 | Asset  | **Turtle sprite sheet** — Same pose set as Sparrow (flying, bank L/R, boost, idle, firing, damage, hit flash); 256×256 cells. Per [sparrow_sprite_sheet_spec](docs/concepts/p0_mocks/p0_1_ships/sparrow/sparrow_sprite_sheet_spec.md).                   | 6.T.1        | Pending | Matches pilot-style canonical art        | Inspect Turtle sprites; CEO approves                |
| 6     | 6.T.6 | Tech   | **Turtle entity + weapons + shield** — TurtleShip class; ShipData; stats per design lock; sprite; primary + secondary weapons; shield per design. Integrate into Gameplay; verify feel.                                                                    | 6.T.1–6.T.5  | Pending | Turtle playable; distinct from Sparrow   | Play Turtle; weapons + shield feel distinct         |
| *Wolf* | | | | | | |
| 7     | 6.W.1 | Design | **Wolf design lock** — Stats (balanced); fighter jet silhouette; white/silver propulsion. Output: `docs/concepts/p0_mocks/p0_1_ships/wolf/wolf_design_lock.md`.                                                                                         | —            | Pending | Per p0_1_ships; design_system            | Read wolf_design_lock.md; CEO approves              |
| 8     | 6.W.2 | Design | **Wolf primary weapon design** — Main gun type, fire pattern, damage, mana cost. Balanced fighter feel.                                                                                     | 6.W.1        | Pending | Combat Systems; balanced fighter feel    | Wolf primary weapon spec approved                  |
| 9     | 6.W.3 | Design | **Wolf secondary weapon design** — Alt fire; cooldown, mana cost, effect. Distinct from primary.                                                                                          | 6.W.1        | Pending | Combat Systems; balanced fighter feel    | Wolf secondary weapon spec approved                 |
| 10    | 6.W.4 | Design | **Wolf shield design** — Shield strength, recharge rate, visual language; aether/filigree per art_style_guide.                                                                               | 6.W.1        | Pending | Combat Systems + Visual Design spec      | Wolf shield spec approved                           |
| 11    | 6.W.5 | Asset  | **Wolf sprite sheet** — Same pose set as Sparrow. Per sprite sheet spec.                                                                                                                                                                                | 6.W.1        | Pending | Matches pilot-style canonical art        | Inspect Wolf sprites; CEO approves                  |
| 12    | 6.W.6 | Tech   | **Wolf entity + weapons + shield** — WolfShip class; ShipData; stats per design lock; sprite; primary + secondary weapons; shield per design. Integrate; verify feel.                                                                                   | 6.W.1–6.W.5  | Pending | Wolf playable; distinct from Sparrow/Turtle | Play Wolf; weapons + shield feel distinct        |
| *Dragon* | | | | | | |
| 13    | 6.D.1 | Design | **Dragon design lock** — Stats (high Attack/Mana, low Defense); multi-gun silhouette; orange/red (#FF4500); compact. Output: `docs/concepts/p0_mocks/p0_1_ships/dragon/dragon_design_lock.md`.                                                          | —            | Pending | Per p0_1_ships; design_system            | Read dragon_design_lock.md; CEO approves            |
| 14    | 6.D.2 | Design | **Dragon primary weapon design** — Main gun type, fire pattern, damage, mana cost. Glass-cannon, multi-gun feel.                                                                            | 6.D.1        | Pending | Combat Systems; glass-cannon feel        | Dragon primary weapon spec approved                 |
| 15    | 6.D.3 | Design | **Dragon secondary weapon design** — Alt fire; cooldown, mana cost, effect. Distinct from primary.                                                                                        | 6.D.1        | Pending | Combat Systems; glass-cannon feel        | Dragon secondary weapon spec approved               |
| 16    | 6.D.4 | Design | **Dragon shield design** — Shield strength, recharge rate, visual language; aether/filigree per art_style_guide.                                                                            | 6.D.1        | Pending | Combat Systems + Visual Design spec      | Dragon shield spec approved                         |
| 17    | 6.D.5 | Asset  | **Dragon sprite sheet** — Same pose set as Sparrow. Per sprite sheet spec.                                                                                                                                                                              | 6.D.1        | Pending | Matches pilot-style canonical art        | Inspect Dragon sprites; CEO approves                |
| 18    | 6.D.6 | Tech   | **Dragon entity + weapons + shield** — DragonShip class; ShipData; stats per design lock; sprite; primary + secondary weapons; shield per design. Integrate; verify feel.                                                                                 | 6.D.1–6.D.5  | Pending | Dragon playable; distinct from others     | Play Dragon; weapons + shield feel distinct        |
| *Ship selection (ships only)* | | | | | | |
| 19    | 6.S.1 | Design | **Ship selection UI design (ships only)** — Layout: 4 ships; selection flow; visual feedback. No pilots. Output: `docs/concepts/ship_selection_ui_design.md`. Controller-first.                                                                        | —            | Pending | Design system; controller-first          | Read ship_selection_ui_design.md; spec approved     |
| 20    | 6.S.2 | Tech   | **Ship selection (pre-level)** — ShipSelect scene before Gameplay; 4 ships; pick ship → load Gameplay with chosen ship. Per ship_selection_ui_design. Ships only; no pilot selection.                                                                    | 6.T.6, 6.W.6, 6.D.6, 6.S.1  | Pending | Pick ship; correct ship in level         | ship select → pick → correct ship  |
...
**Delivery order:** Pilot design → portraits → ship selection UI with pilots. Adds pilot selection and modifiers on top of Phase 6 ship selection.
...
| 2     | 7.A.2 | Design | **Ship selection UI design (with pilots)** — Extend ship selection: 4 ships + 4 pilots; selection flow; visual feedback. Output: `docs/concepts/ship_selection_ui_design.md` (pilot section). Controller-first.                                         | 6.S.2, 7.A.1   | Pending | Design system; controller-first          | Read updated ship_selection_ui_design.md; spec approved |
| 3     | 7.A.3 | Asset  | **Pilot portraits** — 256×256 (Speed, Weapon, Defensive, Rookie). Assets in `Assets/Content/Sprites/Pilots/`. Per pilot_mocks_deliverable.                                                                                                              | —            | Pending | Kaladesh aesthetic                       | Inspect pilot portraits; CEO approves              |
...
| Order | ID    | Type   | Deliverable                                                                                                                                                                               | Depends on   | Status  | Gate                             | How to verify                                      |
| ----- | ----- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ------- | -------------------------------- | -------------------------------------------------- |
| 1     | 8.A.1 | Design | **Hangar layout design** — Ship display area; stat display; upgrade panels; flow (select ship → upgrade → ready). Output: `docs/concepts/hangar_layout_design.md`. Controller navigation. | —            | Pending | Design system; controller-first  | Read hangar_layout_design.md; spec approved         |
| 2     | 8.A.2 | Design | **Upgrade economy design** — Stat cost curves; currency sources; persist rules. Output: `docs/concepts/upgrade_economy_design.md`.                                                        | —            | Pending | Combat Systems + design_system   | Read upgrade_economy_design.md; spec approved       |
| 3     | 8.A.3 | Design | **Weapon/bomb upgrade design** — Strong gun (mana cost, damage); bomb (invincibility, AOE, cooldown); upgrade tiers. Output: `docs/concepts/weapon_bomb_upgrade_design.md`.               | —            | Pending | Combat Systems spec              | Read weapon_bomb_upgrade_design.md; spec approved   |
| 4     | 8.A.4 | Asset  | **Hangar UI assets** — Ship display frame; stat bars; upgrade buttons; ornate inventor-fair aesthetic. Assets in `Assets/Content/Sprites/UI/Hangar/`.                                     | 8.A.1        | Pending | Per art_style_guide              | Inspect Hangar assets; CEO approves                |
| 5     | 8.1   | Tech   | **Hangar scene** — Hangar scene; ship display (selected ship model); stat display; upgrade panels per layout design. Flow: select ship → upgrade → ready.                                 | 8.A.1, 8.A.4 | Pending | Ship display; stat display; flow | Hangar → ship display; flow works  |
| 6     | 8.2   | Tech   | **Upgrade system (stats)** — Spend currency to upgrade ship stats; cost curves per economy design; persist to save (or PlayerPrefs placeholder).                                          | 8.1, 8.A.2   | Pending | Spend currency; persist          | upgrade; spend; persist on quit    |
| 7     | 8.3   | Tech   | **Weapon/bomb upgrades** — Strong gun (mana cost, damage per design); bomb (invincibility, AOE, cooldown); upgrade tiers apply in combat.                                                 | 8.2, 8.A.3   | Pending | Strong gun; bomb; upgrades apply | strong gun; bomb; upgrades in combat |


---

## Phase 9: Progression and Meta
...
| Order | ID    | Type   | Deliverable                                                                                                                                                                              | Depends on        | Status  | Gate                                 | How to verify                                      |
| ----- | ----- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------- | ------------------------------------ | -------------------------------------------------- |
| 1     | 9.A.1 | Design | **Pilot XP/level design** — XP curve; level-up modifiers (Speed/Weapon/Defensive/Neutral); per-pilot progression. Output: `docs/concepts/pilot_xp_level_design.md`.                      | —                 | Pending | Combat Systems + Narrative spec      | Read pilot_xp_level_design.md; spec approved        |
| 2     | 9.A.2 | Design | **Loot design** — Drop tables; currency, HP restore, mana; pickup visuals; enemy-type → resource mapping. Output: `docs/concepts/loot_design.md`.                                        | —                 | Pending | Combat Systems spec                  | Read loot_design.md; spec approved                  |
| 3     | 9.A.3 | Design | **Level 2 design** — Industrial theme; parallax layers; wave composition; difficulty ramp vs Level 1. Output: `docs/concepts/level_2_industrial_design.md`. Per level_mock_3_industrial. | —                 | Pending | Level/Encounter + Visual Design spec | Read level_2_industrial_design.md; spec approved   |
| 4     | 9.A.4 | Asset  | **Level 2 assets** — Parallax (industrial pipes, conduits); terrain; Kaladesh industrial aesthetic. Assets in `Assets/Content/Sprites/Level2/`.                                          | 9.A.3             | Pending | Per art_style_guide; level_mocks     | Inspect Level 2 assets; CEO approves                |
| 5     | 9.A.5 | Asset  | **Pickup/loot assets** — Currency, HP, mana pickups; readable at combat scale. Assets in `Assets/Content/Sprites/Pickups/`. Distinct from projectiles.                                   | 9.A.2             | Pending | Distinct from projectiles            | Inspect pickup assets; CEO approves                 |
| 6     | 9.1   | Tech   | **XP and pilot leveling** — Kill → XP; XP curve per design; level-up → modifiers apply. Per-pilot progression.                                                                           | 9.A.1             | Pending | Kill → XP → level-up → modifiers     | kill enemies → XP → level-up       |
| 7     | 9.2   | Tech   | **Currency and loot** — Enemies drop currency/HP/mana per loot design; pickups spawn; player collects. PickupPool or equivalent.                                                         | 9.A.2, 9.A.5      | Pending | Enemies drop; pickups                | enemies drop; collect pickups       |
| 8     | 9.3   | Tech   | **Multiple levels** — Level 2 (industrial) playable; unlock after Level 1; difficulty ramp per design.                                                                                   | 4.4, 9.A.3, 9.A.4 | Pending | Level 2; unlock; ramp difficulty     | complete L1 → L2 unlocks; play L2  |
| 9     | 9.4   | Tech   | **Save system** — Progress persists on quit; XP, currency, upgrades, level unlock. JSON or PlayerPrefs.                                                                                  | 9.1, 9.2, 8.2     | Pending | Progress persists on quit            | progress; quit; reload; persists    |


---

## Phase 10: Multiplayer (2–4 Players)
...
| Order | ID     | Type   | Deliverable                                                                                                                                                                  | Depends on         | Status  | Gate                                | How to verify                                      |
| ----- | ------ | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------- | ----------------------------------- | -------------------------------------------------- |
| 1     | 10.A.1 | Design | **Combined ship design (2)** — Pilot + gunner; merged silhouette; stat combination formula; merge/split animation intent. Output: `docs/concepts/combined_ship_2_design.md`. | —                  | Pending | Combat Systems + Visual Design spec | Read combined_ship_2_design.md; spec approved      |
| 2     | 10.A.4 | Asset  | **Combined ship assets (2)** — 2-ship merge visuals; combination of constituent ships; propulsion blend. Per art_style_guide.                                                | 10.A.1             | Pending | Distinct from solo ships            | Inspect combined ship assets; CEO approves         |
| 3     | 10.1   | Tech   | **Local 2-player** — Two PlayerShipController instances; separate input (Player 1/2); no input conflict; both ships in Gameplay scene.                                       | 6.S.2                | Pending | 2 ships; no input conflict          | 2 controllers → both move/shoot    |
| 4     | 10.2   | Tech   | **Ship combining (2)** — Merge action: 2 ships → 1 combined; pilot + gunner roles; split action; stat combination per design.                                                | 10.1, 10.A.1, 10.A.4 | Pending | Pilot + gunner; merge/split         | merge 2 ships; pilot+gunner; split  |
| 5     | 10.A.2 | Design | **Combined ship design (3)** — Pilot + gunner + shielder; shield mechanic; role assignment flow. Output: `docs/concepts/combined_ship_3_design.md`.                          | —                  | Pending | Per design_system roles             | Read combined_ship_3_design.md; spec approved      |
| 6     | 10.A.5 | Design | **Shield VFX design** — Shielder role; movable shield; visual language (aether, filigree). Output: `docs/concepts/shield_vfx_design.md`.                                     | —                  | Pending | Visual Design spec                  | Read shield_vfx_design.md; spec approved           |
| 7     | 10.3   | Tech   | **3-player combine** — 3 ships → 1 combined; pilot + gunner + shielder; shield mechanic; role assignment.                                                                    | 10.2, 10.A.2, 10.A.5 | Pending | Pilot + gunner + shielder           | 3 players merge; shielder role      |
| 8     | 10.A.3 | Design | **Combined ship design (4)** — 2 gunners, 1 shielder, 1 pilot (or 2×2); role swap UX. Output: `docs/concepts/combined_ship_4_design.md`.                                     | —                  | Pending | Per design_system                   | Read combined_ship_4_design.md; spec approved      |
| 9     | 10.4   | Tech   | **4-player combine** — 4 ships → 1 combined; 2 gunners, 1 shielder, 1 pilot; roles per design.                                                                               | 10.3, 10.A.3       | Pending | 2 gunners, 1 shielder, 1 pilot      | 4 players merge; all roles         |
| 10    | 10.5   | Tech   | **Role swap** — Swap roles during combined ship; UX per combined design (4).                                                                                                 | 10.4               | Pending | Swap roles during combined          | combined ship → swap roles         |


---

## Phase 11: Content and Polish
...
| Order | ID     | Type   | Deliverable                                                                                                                                                                                                                   | Depends on             | Status  | Gate                                 | How to verify                                      |
| ----- | ------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | ------- | ------------------------------------ | -------------------------------------------------- |
| 1     | 11.A.1 | Design | **Medium enemy design** — HP, defense, attack; distinct behavior vs Scout; insectoid silhouette; size hierarchy. Output: `docs/concepts/medium_enemy_design_lock.md`.                                                         | —                      | Pending | Combat Systems + Visual Design spec  | Read medium_enemy_design_lock.md; spec approved     |
| 2     | 11.A.2 | Design | **Elite enemy design** — Higher stats; unique ability; mini-boss feel; rewarding resources. Output: `docs/concepts/elite_enemy_design_lock.md`. Per [boss_mocks](docs/concepts/p0_mocks/p0_4_boss/).                          | —                      | Pending | Per boss_mocks                       | Read elite_enemy_design_lock.md; spec approved      |
| 3     | 11.B.1 | Design | **Root-Seeker (forest) design** — 2+ phases; firing patterns; unique abilities. Output: `docs/concepts/root_seeker_design_lock.md`. Per [boss_mocks_deliverable](docs/concepts/p0_mocks/p0_4_boss/boss_mocks_deliverable.md). | —                      | Pending | Level/Encounter + Visual Design spec | Read root_seeker_design_lock.md; spec approved     |
| 4     | 11.B.2 | Design | **Conduit-Crawler (industrial) design** — 2+ phases; distinct from Root-Seeker. Output: `docs/concepts/conduit_crawler_design_lock.md`.                                                                                       | —                      | Pending | Per boss_mocks_deliverable           | Read conduit_crawler_design_lock.md; spec approved |
| 5     | 11.C.1 | Design | **VFX design spec** — Projectiles (bright cores, trails); explosions (starburst, flares); hit feedback; 60 FPS target. Output: `docs/concepts/vfx_design_spec.md`.                                                            | —                      | Pending | Per art_style_guide VFX language     | Read vfx_design_spec.md; spec approved              |
| 6     | 11.C.3 | Design | **Audio design** — SFX list (fire, hit, explosion, pickup, UI); music (menu, combat, boss); no pops. Output: `docs/concepts/audio_design.md`.                                                                                 | —                      | Pending | Platform/Release spec                | Read audio_design.md; spec approved                 |
| 7     | 11.D.1 | Design | **Mission briefings design** — Per-level briefing flow; pilot VO hooks. Per [sample_narrative](docs/concepts/p0_mocks/p0_6_narrative/sample_narrative.md). Output: `docs/concepts/mission_briefings_design.md`.               | —                      | Pending | Narrative spec                       | Read mission_briefings_design.md; spec approved     |
| 8     | 11.D.2 | Design | **Pilot bios** — Short bios for Speed, Weapon, Defensive, Rookie; personality, backstory. Output: `docs/concepts/pilot_bios.md`.                                                                                              | —                      | Pending | Narrative spec; per pilot_mocks      | Read pilot_bios.md; spec approved                   |
| 9     | 11.A.3 | Asset  | **Medium/Elite sprite sheets** — Top-down insectoid; amber/olive-green/purple-grey palette. Individual sprites in `Assets/Content/Sprites/Enemies/`.                                                                          | 11.A.1, 11.A.2         | Pending | Per art_style_guide                  | Inspect Medium/Elite sprites; CEO approves         |
| 10    | 11.B.3 | Asset  | **Boss sprite sheets** — Root-Seeker, Conduit-Crawler; multi-segment; biomechanical; phase transitions. Assets in `Assets/Content/Sprites/Bosses/`.                                                                           | 11.B.1, 11.B.2         | Pending | Imposing scale                       | Inspect boss sprites; CEO approves                  |
| 11    | 11.C.2 | Asset  | **VFX assets** — Player/enemy projectiles; explosions; muzzle flashes; damage sparks. Lightweight, pool-friendly.                                                                                                             | 11.C.1                 | Pending | Lightweight, performant              | Inspect VFX assets; CEO approves                    |
| 12    | 11.1   | Tech   | **Enemy tiers (Medium, Elite)** — MediumEnemy, EliteEnemy prefabs; distinct HP, behaviors per design locks; replace or extend Scout.                                                                                          | 11.A.1, 11.A.2, 11.A.3 | Pending | Distinct HP, behaviors               | Medium/Elite appear; distinct       |
| 13    | 11.2   | Tech   | **Boss variety** — Root-Seeker, Conduit-Crawler prefabs; 2+ phases each; firing patterns per design locks. Replace Phase 4 placeholder.                                                                                       | 11.B.1, 11.B.2, 11.B.3 | Pending | 2+ phases per boss                   | both bosses; 2+ phases each         |
| 14    | 11.3   | Tech   | **VFX pass** — Projectiles, explosions, hit feedback per VFX design spec; 60 FPS. Replace placeholders with approved assets.                                                                                                  | 11.C.1, 11.C.2         | Pending | Projectiles, explosions; 60 FPS      | VFX visible; 60 FPS                 |
| 15    | 11.4   | Tech   | **Audio** — SFX (fire, hit, explosion, pickup, UI); music (menu, combat, boss); no pops. Per audio design.                                                                                                                    | 11.C.3                 | Pending | SFX, music; no pops                  | SFX/music play; no pops             |
| 16    | 11.5   | Tech   | **Narrative integration** — Mission briefings before levels; pilot bios in ship select/hangar. Per mission briefings + pilot bios design.                                                                                     | 11.D.1, 11.D.2         | Pending | Briefings, pilot bios                | briefings; pilot bios visible       |


---

## Phase 12: Platform and Release
...
| Order | ID   | Type | Deliverable                                                                                                         | Depends on       | Status  | Gate                        | How to verify                                      |
| ----- | ---- | ---- | ------------------------------------------------------------------------------------------------------------------- | ---------------- | ------- | --------------------------- | -------------------------------------------------- |
| 1     | 12.1 | Tech | **Steam build** — Steamworks SDK integration; build runs on Steam; achievements/cloud optional. Target: Windows PC. | 11.5             | Pending | Runs on Steam               | Build; run on Steam; verify launch                 |
| 2     | 12.2 | Tech | **Mac build** — Full playthrough on Mac; resolution, input, no macOS-specific crashes.                              | 12.1             | Pending | Full playthrough on Mac     | Build; full playthrough on Mac; no crashes          |
| 3     | 12.3 | Tech | **Switch build** — Handheld 720p; docked 1080p; Nintendo SDK; controller compliance; performance targets.           | 12.1             | Pending | Handheld 720p; docked 1080p | Build; test handheld + docked; verify res          |
| 4     | 12.4 | Tech | **CI/CD** — Push to main → build artifact; GitHub Actions or equivalent; build verification.                        | —                | Pending | Push → build artifact       | Push to main; check Actions; artifact produced     |
| 5     | 12.5 | Tech | **Release checklist** — QA pass; store assets (screenshots, trailer, description); submit to Steam/Switch.          | 12.1, 12.2, 12.3 | Pending | QA; store assets; submit    | QA pass; store assets ready; submit complete       |
