...
| `miniboss` | object \| null | No | Mini-boss config. Null = no mini-boss. |
...
| `count` | number | Yes | Total enemies in wave. Overrides squads × enemiesPerSquad if both present; otherwise derived. |
...
|-------|------|-------------|
| `preMiniBossSeconds` | number \| null | Game time (or scroll-equivalent) when mini-boss spawns. Null = no mini-boss, or spawn after N waves. |
| `preBossSeconds` | number \| null | Game time when boss spawns. Null = spawn after all waves complete (current behavior). |
...
| `phases` | number | No | Number of phases. Default from archetype. |
