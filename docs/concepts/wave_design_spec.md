...
**CEO names:** Formations can be requested by attack-pattern name in briefs and level JSON. V (Wedge) = **Wedge Assault**; Pincer = **Pincer Assault**. Full list: [attack_pattern_reference.md](../design_system/attack_pattern_reference.md). Each pattern also drives **movement behavior** (zig-zag, swoop, scatter-converge, etc.) per `src/enemies/scout-movement.ts`.
